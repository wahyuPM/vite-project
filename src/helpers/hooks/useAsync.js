...
import useSafeDispacth from './useSafeDispacth'
...
export default function useAsync(initialState) {
    ...
    const safeSetState = useSafeDispacth(setState)
    ...
    return {
        data, status, error, run, setData, setError, reset, isIdle: status === 'idle', isLoading: status === 'idle' || status === 'pending', isError: status === 'rejected', isSuccess: status === 'resolved'
    }
}
